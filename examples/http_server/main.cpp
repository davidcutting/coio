...
// A homogeneous pool of io_executor worker threads — this is kioto's runtime layer; the old hand-rolled
...
using server_runtime = kioto::basic_runtime<http::io_executor>;

auto signal_watchdog(kioto::async_scope& scope) -> kioto::inline_task<> {
    const int signum = co_await kioto::signal_wait(SIGINT, SIGTERM);
    ::println("server stop with signal: ({}){}", signum, kioto::strsignal(signum));
...
auto start_server(server_runtime& pool, kioto::async_scope& scope, http::router& router) -> http::io_executor::task<> try {
    http::tcp_acceptor acceptor(co_await kioto::read_scheduler());
    acceptor.open(kioto::tcp::v6());
    acceptor.set_option(http::tcp_acceptor::reuse_address(true));
    acceptor.set_option(http::tcp_acceptor::v6_only(false));
    acceptor.bind({kioto::ipv6_address::any(), port});
    acceptor.listen();
