...
namespace coio::detail {
    // A per-thread, size-classed free-list that recycles coroutine frames. NOT thread-safe by design:
    // each worker owns one and only ever touches its own (via tl_frame_pool). A frame allocated on one
    // thread but freed on another is returned to the FREEING thread's pool — blocks migrate, which is
    // safe because every block is ::operator new-backed, so any pool can ::operator delete any block at
    // shutdown. For a PINNED task (born + dies on one worker) alloc and free are the same thread, so it
    // is a plain pop/push with no atomics.
    class frame_pool {
    public:
