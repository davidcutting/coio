...
// A reference run-loop metrics sink for basic_executor<Metrics, Drivers...>. The default policy is
// coio::no_metrics (in execution_context.h) — zero cost, zero size. Plug counting_metrics in to record the
// executor's activity; a monitor thread reads snapshot(). Driver-level metrics (SQE/CQE/enter counts) and
// export/aggregation are separate, later concerns.
namespace coio {
    // A snapshot of one executor's run-loop counters. Rates (throughput, utilization, wake rate, fast-path
    // ratio) come from diffing two snapshots over an interval.
    ...
    // Percentiles of a latency sample stream. Values are POWER-OF-TWO bucket lower edges: a reported p99 of
    // 8us means the true 99th percentile is in [8us, 16us) — ~2x resolution, the standard log-histogram
    // trade for zero allocation and O(1) record. count is the number of samples behind these figures.
    struct latency_stats {
        std::uint64_t count;
        std::chrono::nanoseconds p50, p90, p99, max;
    };

    // A fixed, allocation-free log2 latency histogram: bucket i counts samples in [2^i, 2^(i+1)) ns. record()
    // is a single relaxed fetch_add (off the hot path — fed by sampled probes, not every op), so it is safe
    // for the handful of probe completions that may land concurrently on different workers. snapshot() reads
    // relaxed (torn reads only shift a percentile by a bucket, which is monitoring-grade). CUMULATIVE — a
    // per-interval view comes from diffing two snapshots' bucket counts (a later refinement).
    ...
    // Owner-thread hooks use single-writer relaxed load+store (a plain mov on x86, NOT a lock-prefixed RMW)
    // so they're ~free on the hot path; the two genuinely cross-thread hooks (woke/submitted_remote) use
    // fetch_add, but only fire on already-slow paths. snapshot() reads relaxed (torn reads fine for monitoring).
    class counting_metrics {
    public:
        // ---- owner-thread hooks (single writer) ----
        COIO_ALWAYS_INLINE void turn() noexcept { bump(turns_); }
        COIO_ALWAYS_INLINE void ran(std::size_t n) noexcept { add(ops_run_, n); }
        COIO_ALWAYS_INLINE void parked() noexcept { bump(parks_); park_start_ = clock::now(); }
        ...
        COIO_ALWAYS_INLINE void submitted_local() noexcept { bump(submits_local_); }

        // ---- any-thread hooks (multi writer) ----
        COIO_ALWAYS_INLINE void woke() noexcept { wakes_.fetch_add(1, rlx); }
        COIO_ALWAYS_INLINE void submitted_remote() noexcept { submits_remote_.fetch_add(1, rlx); }
    ...
    private:
        ...
        COIO_ALWAYS_INLINE static void bump(std::atomic<std::size_t>& c) noexcept { c.store(c.load(rlx) + 1, rlx); }
        COIO_ALWAYS_INLINE static void add(std::atomic<std::size_t>& c, std::size_t n) noexcept { c.store(c.load(rlx) + n, rlx); }
