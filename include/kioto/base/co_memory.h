...
namespace kioto::detail {
    ...
    KIOTO_ALWAYS_INLINE constexpr auto ceiling_division(std::size_t n, std::size_t m) noexcept -> std::size_t {
        KIOTO_ASSERT(m != 0);
        return (n + m - 1) / m;
    }

    KIOTO_ALWAYS_INLINE constexpr auto ceiling_align(std::size_t n, std::size_t m) noexcept -> std::size_t {
        KIOTO_ASSERT(m != 0);
        return ceiling_division(n, m) * m;
    }

    KIOTO_ALWAYS_INLINE auto align_address(void* ptr, std::size_t alignment, std::size_t least_offset) noexcept -> void* {
    ...
    struct promise_alloc_control {
        ...
        auto operator new (std::size_t n, std::allocator_arg_t, const OtherAlloc& other_alloc, const Args&...) -> void* { // for normal corotuine function `auto some_function(std::allocator_arg_t, allocator, ...) -> kioto::task<...>`
        ...
        auto operator new (std::size_t n, const This&, std::allocator_arg_t, const OtherAlloc& other_alloc, const Args&...) -> void* { // for non-static member corotuine function `auto some_class::some_function(std::allocator_arg_t, allocator, ...) -> kioto::task<...>`
