...
// Capability tags name SERVICES, not drivers: several drivers may provide one (timer heap, epoll timerfd,
// io_uring timeout all provide `timer`). A driver advertises `using capabilities = type_list<...>`;
// executor::get_driver<Cap> resolves to the first (declaration order) driver providing Cap.
namespace kioto::capability {
    ...
    // A ROLE, not an op-family: a dedicated reactor-less worker (park/wake + run posted work). Only the
    // lightweight worker driver claims it, so .capability<cpu>() resolves there, not to a heavy reactor.
    struct cpu {};
}
