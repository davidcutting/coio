...
#include <utility>
#include <vector>
#include <kioto/core.h>                 // just / then / sync_wait / execution::scheduler
#include <kioto/runtime/metrics.h>
#include <kioto/exec/async_scope.h>    // async_scope (probe lifetime)

// Metrics export: a side-thread (or caller-driven) scraper that turns a metered runtime's monotonic
// per-worker snapshots into RATES by diffing successive samples. Entirely off the hot path (only
// collect_metrics(), relaxed reads). Sink is a user callback — kioto owns no HTTP/OTel dependency.
//
//     kioto::metrics_reporter reporter{rt};                 // baseline snapshot taken here
//     reporter.run_every(1s, [](const kioto::runtime_report& r) {
...
namespace kioto {
    ...
    // Owns a runtime reference + the previous sample. NON-movable: run_every()'s thread captures `this`, so
    // pin it (a named local). Single-consumer: tick() mutates the baseline — call it from ONE place.
    ...
    public:
        ...
        ~metrics_reporter() {
            stop();
            kioto::this_thread::sync_wait(probe_scope_.join());
        }
        ...
        // Sample submit->run scheduling latency: a side thread fires a self-timing probe every `interval`,
        // recording (run - submit) into the histogram (surfaced as scheduling_latency in tick()). Capped at
        // `max_in_flight` outstanding, so a saturated runtime throttles the probe rate itself (backpressure).
    ...
    private:
        ...
        auto fire_probe() -> void {
            ...
            probe_scope_.spawn_on(rt_->get_scheduler(), kioto::just() | kioto::then([this, t0]() noexcept {
