...
#include <kioto/runtime/metrics.h>           // executor_stats (runtime-level aggregation)
...
namespace kioto {
    ...
    public:
        ...
        public:
            ...
            public:
                ...
                template<execution::receiver Rcvr>
                KIOTO_ALWAYS_INLINE auto connect(Rcvr rcvr) && noexcept -> operation<Rcvr> {
            ...
            [[nodiscard]]
            KIOTO_ALWAYS_INLINE static constexpr auto query(execution::get_forward_progress_guarantee_t) noexcept {
                return execution::forward_progress_guarantee::parallel;
            }

            [[nodiscard]]
            KIOTO_ALWAYS_INLINE auto query(get_allocator_t) const noexcept -> std::pmr::polymorphic_allocator<> {
                return {};
            }
    ...
    public:
        ...
        ~basic_runtime() {
            // The scope must be joined once before destruction. If join() wasn't driven, cancel + drain it
            // WHILE the workers still run (suspended I/O needs them), then stop.
        ...
        // A default (no_metrics) worker doesn't offer these, so an unmetered runtime stays zero-cost.
        ...
        // Round-robin a specific worker's own scheduler, for pinning CONTEXT-AFFINE work (bind an io_object
        // to worker i and keep it there). Unlike get_scheduler() (round-robins bare ops), returns a concrete worker.
