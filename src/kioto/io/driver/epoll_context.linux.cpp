...
namespace kioto {
    namespace detail {
        driver_interrupter::driver_interrupter() {
    ...
    auto epoll_driver::operation::register_event(int event_type, std::uint32_t extra_flags) noexcept -> bool {
        ...
        if (event_type == EPOLLIN) {
            KIOTO_ASSERT(not in_op_registered && "input op initiated before a prior input op completed.");
        }
        else if (event_type == EPOLLOUT) {
            KIOTO_ASSERT(not out_op_registered && "output op initiated before a prior output op completed.");
        }
        else unreachable();
    ...
    // Drop op's registration; returns whether it was still registered. registered_event_ is read HERE under
    // the fd lock — reading it before the lock races the owner's register_event. (Caller posts the op back.)
    auto epoll_driver::deregister(operation* op) -> bool {
        KIOTO_ASSERT(op != nullptr and op->data != nullptr);
        ...
        KIOTO_ASSERT(registered_op == nullptr or registered_op == op);
    ...
    namespace detail {
        ...
        // Stream (TCP) receive: same readiness path as async_receive_t, but eof_on_zero folds a 0-byte recv
        // on a non-empty buffer into error::eof (peer closed).
        ...
        // Stream/datagram send share the ::send readiness path; separate descriptions only so stoppability
        // is deduced. Identical bodies today.
