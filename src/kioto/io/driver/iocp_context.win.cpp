...
#pragma clang diagnostic pop
    ...
    namespace detail {
        ...
        // do_start() convention: true = async completion pending (fed to the port); false = completed
        // synchronously with `result` set (op-state posts itself). IOCP queues a packet even for
        // synchronously-successful overlapped calls, so those return true; only pre-syscall failures and
        // empty-buffer no-ops return false.
        // TODO: async file ops using FILE_SKIP_COMPLETION_PORT_ON_SUCCESS notification mode
        ...
        // async_stream_receive: same WSARecv as async_receive, but eof_on_zero folds a 0-byte recv on a
        // non-empty buffer into error::eof (TCP peer closed).
        template<>
        ...
        // (datagram = unstoppable). Identical bodies today.
        template<>
