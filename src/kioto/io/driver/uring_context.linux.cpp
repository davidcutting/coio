...
namespace kioto {
    namespace {
        ...
        auto init_uring(::io_uring& uring, std::size_t entries) -> void {
            ...
            // R_DISABLED: the single-issuer owner binds when the ring is enabled on the run() thread.
            // SUBMIT_ALL: a bad SQE doesn't abort the rest of the batch. COOP_TASKRUN only on the non-DEFER
            // fallback (DEFER_TASKRUN already implies cooperative task-run, so COOP with it is redundant).
    ...
    auto uring_driver::operation::do_cancel() -> void {
        ...
        // Cross-thread stop: never touch the ring off-owner. Hand the op to the owner to submit the cancel;
        // cancel_state_ keeps the node alive until its CQE is processed.
    ...
    auto uring_driver::classify(detail::ready_queue& ready, void* user_data, int res, unsigned flags) -> void {
        ...
        KIOTO_TSAN_ACQUIRE(op);
        // Multishot: on_completion returns false while still armed (F_MORE), so we don't finish/free until true.
    ...
    auto uring_driver::poll(detail::ready_queue& ready, std::size_t batch) -> void {
        if (not enabled_) [[unlikely]] {
            ...
            // Register the ring fd so each io_uring_enter passes a registered index, skipping the per-enter
            // fd->file lookup. Best-effort (older kernels lack it; the raw fd works regardless — and the
            // cross-thread wake targets uring_.ring_fd via register_sync_msg, a separate path).
    ...
    auto uring_driver::poll_wait() -> void {
        // One enter flushes pending SQEs AND blocks for >=1 CQE (TIMEOUT SQEs wake it; no deadline needed).
        // Returns the SQEs submitted (like io_uring_submit), so reconcile pending_sqes_ the same way.
        int ret = 0;
        do {
        ...
        if (ret > 0) {
            KIOTO_ASSERT(pending_sqes_ >= std::size_t(ret)); // NOLINT(*-use-integer-sign-comparison)
            pending_sqes_ -= ret;
        }
    ...
    auto uring_driver::submit(operation& op) -> bool {
        ...
        KIOTO_TSAN_RELEASE(static_cast<operation*>(&op));
        post_submit_sqes();
        return true;
    }
    ...
    auto uring_driver::submit_sqes() -> void { // owner thread only
        ...
        KIOTO_ASSERT(pending_sqes_ >= std::size_t(n)); // NOLINT(*-use-integer-sign-comparison)
        pending_sqes_ -= n;
    }
    ...
    namespace detail {
        ...
        // Stream/datagram send share prep_send; separate descriptions so stoppability is deduced (datagram
        // = unstoppable). Identical bodies today.
        ...
        template<> auto uring_state_base_for<async_accept_t>::on_completion(int cqe_res, unsigned) -> bool {
            ...
            // Wrap the freshly-minted fd into an opaque handle right at the mint site (the driver boundary),
